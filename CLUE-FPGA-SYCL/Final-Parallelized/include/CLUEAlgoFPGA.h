...
// width of the on-chip parallel lanes used to scan a tile bin; tune down if area estimate overflows the device
static constexpr int kBinUnroll = 1;
//new
            ...
            for(int i = 0 ; i < numberOfPoints ; ++i){
                ...
                for(int xBin = search_box.x() ; xBin < search_box.y() + 1 ; ++xBin){
                    ...
                    for(int yBin = search_box.z() ; yBin < search_box.w() + 1 ; ++yBin){
                        ...
                        static_assert(T::maxTileDepth % kBinUnroll == 0, "kBinUnroll must divide maxTileDepth");
                        float rho2 = 0;
                        // process the bin in kBinUnroll-wide chunks: fully parallel within a chunk, pipelined across chunks
                        [[intel::initiation_interval(1)]]
                        for (int chunk = 0; chunk < T::maxTileDepth / kBinUnroll; ++chunk) {
                            [[intel::fpga_register]] int   j_local[kBinUnroll];
                            [[intel::fpga_register]] float xj_local[kBinUnroll];
                            [[intel::fpga_register]] float yj_local[kBinUnroll];
                            [[intel::fpga_register]] float wj_local[kBinUnroll];
                            #pragma unroll
                            for (int b = 0; b < kBinUnroll; ++b) {
                                int j = lt[binId][chunk * kBinUnroll + b];
                                j_local[b]  = j;
                                xj_local[b] = x_d[j];
                                yj_local[b] = y_d[j];
                                wj_local[b] = w_d[j];
                            ...
                            #pragma unroll
                            for (int b = 0; b < kBinUnroll; ++b) {
                                bool valid = (chunk * kBinUnroll + b) < binSize;
                                int j = j_local[b];
                                float dist_ij_2 = ((xi - xj_local[b]) * (xi - xj_local[b])
                                                + (yi - yj_local[b]) * (yi - yj_local[b]));
                                rho2 += valid * (dist_ij_2 <= dc*dc) * (i == j ? 1.f : 0.5f) * wj_local[b];
                            }
                        }
                        rho1 += rho2 ;
                    }
                ...
                for(int xBin = search_box.x() ; xBin < search_box.y() + 1 ; ++xBin){
                    ...
                    for(int yBin = search_box.z() ; yBin < search_box.w() + 1 ; ++yBin){
                        ...
                        for (int chunk = 0; chunk < T::maxTileDepth / kBinUnroll; ++chunk) {
                            ...
                            [[intel::fpga_register]] float xj_local[kBinUnroll];
                            [[intel::fpga_register]] float yj_local[kBinUnroll];
                            [[intel::fpga_register]] float rhoj_local[kBinUnroll];
                            #pragma unroll
                            for (int b = 0; b < kBinUnroll; ++b) {
                                int j = lt[binId][chunk * kBinUnroll + b];
                                j_local[b]   = j;
...
void CLUEAlgoFPGA<T,NLAYERS>::makeClusters(){
    ...
    kernel_find_clusters(q,d_seeds,d_points,outlierDeltaFactor_,dc_,rhoc_, d_parentA, points_.n);
    // // }
    // // else{
        // kernel_find_clusters_kappa(q,d_seeds,d_followers,d_points,outlierDeltaFactor_,kappa_,rhoc_,points_.n);
    // // }
