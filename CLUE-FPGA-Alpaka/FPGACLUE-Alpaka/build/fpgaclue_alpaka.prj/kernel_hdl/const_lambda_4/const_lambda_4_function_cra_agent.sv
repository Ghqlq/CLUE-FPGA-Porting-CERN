...
module const_lambda_4_function_cra_agent (
    ...
    output wire [1471:0] kernel_arguments,
    ...
    wire [1471:0] kernel_arg_bit_join_q;
    ...
    reg [1471:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_kernel_start_q;
    reg [1471:0] pipe_reg1_kernel_arguments_q;
    ...
    // bit_enable_bottom(BITSELECT,179)
    ...
    // write_selector_start_status(MUX,317)
    ...
    // printf_bit(BITSELECT,275)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,276)
    ...
    // finish_counter_NO_SHIFT_REG(REG,207)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // readdata_lower_bits_mux(MUX,281)
    ...
    // cra_output_readdata_reg(REG,203)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,559)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // cra_output_readdatavalid_reg(REG,204)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_12_NO_NAME_x(LOGICAL,393)
    ...
    // bus_high(BITSELECT,184)
    assign bus_high_b = avs_cra_writedata[63:32];

    // bit_enable_top(BITSELECT,181)
    ...
    // bit_enable_top_bar(BITSELECT,182)
    ...
    // global_offset_reg_upper_0_buffered(REG,224)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,223)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0_buffered(REG,218)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,217)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1_buffered(REG,226)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,225)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1_buffered(REG,220)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,219)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_14_NO_NAME_x(LOGICAL,405)
    ...
    // global_offset_reg_upper_2_buffered(REG,228)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,227)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2_buffered(REG,222)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,221)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0_buffered(REG,233)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,232)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1_buffered(REG,235)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,234)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2_buffered(REG,237)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,236)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_21_address_ref_x(CONSTANT,448)
    ...
    // dupName_21_can_write_x(LOGICAL,449)
    ...
    // dupName_20_address_ref_x(CONSTANT,442)
    ...
    // dupName_18_address_ref_x(CONSTANT,430)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,355)
    ...
    // dupName_20_NO_NAME_x(LOGICAL,441)
    ...
    // dupName_19_NO_NAME_x(LOGICAL,435)
    ...
    // dupName_2_arg_bit_join_x(BITJOIN,334)
    ...
    // dupName_0_arg_bit_join_x(BITJOIN,322)
    assign dupName_0_arg_bit_join_x_q = {arguments_5_q, arguments_4_q};

    // dupName_4_address_ref_x(CONSTANT,343)
    ...
    // kernel_arg_bit_join(BITJOIN,246)
    assign kernel_arg_bit_join_q = {arguments_45_q, dupName_5_arg_bit_join_x_q, dupName_4_arg_bit_join_x_q, dupName_3_arg_bit_join_x_q, dupName_2_arg_bit_join_x_q, arguments_8_q, dupName_1_arg_bit_join_x_q, dupName_0_arg_bit_join_x_q, arg_bit_join_q, arguments_1_q, arguments_0_q};

    // pipe_reg0_kernel_arguments(REG,271)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,273)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg0_kernel_start(REG,272)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_start(REG,274)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // local_size_reg_0_buffered(REG,249)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_1_buffered(REG,251)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_2_buffered(REG,253)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_0_buffered(REG,265)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_1_buffered(REG,267)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_2_buffered(REG,269)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_NO_NAME_x(LOGICAL,357)
    ...
    // work_dim_NO_SHIFT_REG_buffered(REG,314)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG_buffered(REG,316)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,315)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
