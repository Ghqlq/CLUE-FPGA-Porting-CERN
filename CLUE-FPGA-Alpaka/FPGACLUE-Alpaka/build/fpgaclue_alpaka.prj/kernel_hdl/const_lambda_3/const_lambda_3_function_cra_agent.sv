...
module const_lambda_3_function_cra_agent (
    ...
    reg [1567:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_kernel_start_q;
    reg [1567:0] pipe_reg1_kernel_arguments_q;
    ...
    // bit_enable_bottom(BITSELECT,185)
    ...
    // write_selector_start_status(MUX,323)
    ...
    // printf_bit(BITSELECT,281)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,282)
    ...
    // finish_counter_NO_SHIFT_REG(REG,213)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // readdata_lower_bits_mux(MUX,287)
    ...
    // cra_output_readdata_reg(REG,209)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,582)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // cra_output_readdatavalid_reg(REG,210)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // bus_high(BITSELECT,190)
    assign bus_high_b = avs_cra_writedata[63:32];

    // bit_enable_top(BITSELECT,187)
    ...
    // bit_enable_top_bar(BITSELECT,188)
    ...
    // global_offset_reg_upper_0_buffered(REG,230)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,229)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0_buffered(REG,224)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,223)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1_buffered(REG,232)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,231)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1_buffered(REG,226)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,225)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2_buffered(REG,234)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,233)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2_buffered(REG,228)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,227)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_7_NO_NAME_x(LOGICAL,371)
    ...
    // global_size_reg_lower_0_buffered(REG,239)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,238)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1_buffered(REG,241)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,240)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_8_NO_NAME_x(LOGICAL,377)
    ...
    // global_size_reg_lower_2_buffered(REG,243)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,242)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_2_bit_join(BITJOIN,237)
    assign global_size_2_bit_join_q = {global_size_reg_upper_0_q, global_size_reg_lower_2_q};

    // global_size_2(GPOUT,590)
    assign global_size_2 = global_size_2_bit_join_q;

    // dupName_27_address_ref_x(CONSTANT,492)
    assign dupName_27_address_ref_x_q = 6'b101000;
    ...
    // dupName_26_address_ref_x(CONSTANT,486)
    ...
    // dupName_6_arg_bit_join_x(BITJOIN,369)
    assign dupName_6_arg_bit_join_x_q = {arguments_48_q, arguments_47_q, arguments_46_q, arguments_45_q};

    // dupName_43_mask0_x(LOGICAL,554)
    assign dupName_43_mask0_x_q = bit_enable_bottom_bar_b & arguments_44_buffered_q;

    // dupName_43_new_data_x(LOGICAL,556)
    ...
    // dupName_35_NO_NAME_x(LOGICAL,525)
    ...
    // dupName_34_NO_NAME_x(LOGICAL,521)
    ...
    // dupName_33_NO_NAME_x(LOGICAL,517)
    ...
    // dupName_32_NO_NAME_x(LOGICAL,513)
    ...
    // dupName_31_NO_NAME_x(LOGICAL,509)
    ...
    // dupName_30_NO_NAME_x(LOGICAL,505)
    ...
    // dupName_29_NO_NAME_x(LOGICAL,501)
    ...
    // dupName_28_NO_NAME_x(LOGICAL,497)
    ...
    // dupName_26_NO_NAME_x(LOGICAL,485)
    ...
    // dupName_10_address_ref_x(CONSTANT,390)
    ...
    // dupName_10_can_write_x(LOGICAL,391)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,361)
    ...
    // dupName_9_address_ref_x(CONSTANT,384)
    ...
    // dupName_9_can_write_x(LOGICAL,385)
    ...
    // dupName_4_arg_bit_join_x(BITJOIN,354)
    ...
    // dupName_8_address_ref_x(CONSTANT,378)
    ...
    // dupName_7_address_ref_x(CONSTANT,372)
    ...
    // dupName_6_address_ref_x(CONSTANT,364)
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,335)
    assign dupName_1_arg_bit_join_x_q = {arguments_7_q, arguments_6_q};

    // dupName_5_address_ref_x(CONSTANT,356)
    ...
    // dupName_0_arg_bit_join_x(BITJOIN,328)
    assign dupName_0_arg_bit_join_x_q = {arguments_5_q, arguments_4_q};

    // dupName_4_address_ref_x(CONSTANT,349)
    ...
    // dupName_3_address_ref_x(CONSTANT,342)
    ...
    // kernel_arg_bit_join(BITJOIN,252)
    ...
    // pipe_reg0_kernel_arguments(REG,277)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,279)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg0_kernel_start(REG,278)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_start(REG,280)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // local_size_reg_0_buffered(REG,255)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_1_buffered(REG,257)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_2_buffered(REG,259)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_0_buffered(REG,271)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_1_buffered(REG,273)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_2_buffered(REG,275)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_NO_NAME_x(LOGICAL,363)
    ...
    // work_dim_NO_SHIFT_REG_buffered(REG,320)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG_buffered(REG,322)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,321)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
