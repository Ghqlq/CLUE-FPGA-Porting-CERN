...
// SystemVerilog created from const_lambda_1_function_cra_agent
...
module const_lambda_1_function_cra_agent (
    ...
    output wire [1503:0] kernel_arguments,
    ...
    wire [1503:0] kernel_arg_bit_join_q;
    ...
    reg [1503:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_kernel_start_q;
    reg [1503:0] pipe_reg1_kernel_arguments_q;
    ...
    // bit_enable_bottom(BITSELECT,181)
    ...
    // write_selector_start_status(MUX,319)
    ...
    // printf_bit(BITSELECT,277)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,278)
    ...
    // finish_counter_NO_SHIFT_REG(REG,209)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // readdata_lower_bits_mux(MUX,283)
    ...
    // cra_output_readdata_reg(REG,205)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // avs_cra_readdata(GPOUT,569)
    assign avs_cra_readdata = cra_output_readdata_reg_q;

    // cra_output_readdatavalid_reg(REG,206)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_12_NO_NAME_x(LOGICAL,397)
    ...
    // bus_high(BITSELECT,186)
    assign bus_high_b = avs_cra_writedata[63:32];

    // bit_enable_top(BITSELECT,183)
    ...
    // bit_enable_top_bar(BITSELECT,184)
    ...
    // global_offset_reg_upper_0_buffered(REG,226)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,225)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0_buffered(REG,220)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,219)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1_buffered(REG,228)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,227)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1_buffered(REG,222)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,221)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_14_NO_NAME_x(LOGICAL,409)
    ...
    // global_offset_reg_upper_2_buffered(REG,230)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,229)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2_buffered(REG,224)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,223)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_7_NO_NAME_x(LOGICAL,367)
    ...
    // global_size_reg_lower_0_buffered(REG,235)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,234)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1_buffered(REG,237)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_1(REG,236)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_8_NO_NAME_x(LOGICAL,373)
    ...
    // global_size_reg_lower_2_buffered(REG,239)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_2(REG,238)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_arg_bit_join_x(BITJOIN,365)
    assign dupName_6_arg_bit_join_x_q = {arguments_46_q, arguments_45_q};

    // dupName_43_mask0_x(LOGICAL,547)
    assign dupName_43_mask0_x_q = bit_enable_bottom_bar_b & arguments_44_buffered_q;

    // dupName_43_new_data_x(LOGICAL,549)
    ...
    // dupName_24_address_ref_x(CONSTANT,470)
    ...
    // dupName_23_address_ref_x(CONSTANT,464)
    ...
    // dupName_22_address_ref_x(CONSTANT,458)
    ...
    // dupName_21_address_ref_x(CONSTANT,452)
    ...
    // dupName_21_can_write_x(LOGICAL,453)
    ...
    // dupName_20_address_ref_x(CONSTANT,446)
    ...
    // dupName_20_can_write_x(LOGICAL,447)
    ...
    // dupName_19_address_ref_x(CONSTANT,440)
    ...
    // dupName_19_can_write_x(LOGICAL,441)
    ...
    // dupName_18_can_write_x(LOGICAL,435)
    ...
    // dupName_17_address_ref_x(CONSTANT,428)
    ...
    // dupName_17_can_write_x(LOGICAL,429)
    ...
    // dupName_16_address_ref_x(CONSTANT,422)
    ...
    // dupName_16_can_write_x(LOGICAL,423)
    ...
    // dupName_22_NO_NAME_x(LOGICAL,457)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,357)
    ...
    // dupName_21_NO_NAME_x(LOGICAL,451)
    ...
    // dupName_3_arg_bit_join_x(BITJOIN,343)
    ...
    // dupName_6_address_ref_x(CONSTANT,360)
    ...
    // dupName_3_address_ref_x(CONSTANT,338)
    ...
    // kernel_arg_bit_join(BITJOIN,248)
    ...
    // pipe_reg0_kernel_arguments(REG,273)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,275)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg0_kernel_start(REG,274)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_start(REG,276)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // local_size_reg_0_buffered(REG,251)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_1_buffered(REG,253)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // local_size_reg_2_buffered(REG,255)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_0_buffered(REG,267)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_1_buffered(REG,269)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // num_groups_reg_2_buffered(REG,271)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_6_NO_NAME_x(LOGICAL,359)
    ...
    // work_dim_NO_SHIFT_REG_buffered(REG,316)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG_buffered(REG,318)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // workgroup_size_NO_SHIFT_REG(REG,317)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
