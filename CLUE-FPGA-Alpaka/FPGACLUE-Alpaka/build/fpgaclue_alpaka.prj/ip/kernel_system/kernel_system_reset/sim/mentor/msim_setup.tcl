...
# ----------------------------------------
# If kernel_system_reset is one of several IP cores in your
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "kernel_system_reset.kernel_system_reset"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [kernel_system_reset::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [kernel_system_reset::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [kernel_system_reset::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [kernel_system_reset::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [kernel_system_reset::normalize_path "$file"]
  set normalized_dest [kernel_system_reset::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [kernel_system_reset::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [kernel_system_reset::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [kernel_system_reset::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [kernel_system_reset::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
