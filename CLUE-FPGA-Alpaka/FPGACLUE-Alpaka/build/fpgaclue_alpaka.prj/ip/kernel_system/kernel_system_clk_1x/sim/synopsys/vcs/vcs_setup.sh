...
TOP_LEVEL_NAME="kernel_system_clk_1x"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [kernel_system_clk_1x::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {kernel_system_clk_1x::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [kernel_system_clk_1x::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
