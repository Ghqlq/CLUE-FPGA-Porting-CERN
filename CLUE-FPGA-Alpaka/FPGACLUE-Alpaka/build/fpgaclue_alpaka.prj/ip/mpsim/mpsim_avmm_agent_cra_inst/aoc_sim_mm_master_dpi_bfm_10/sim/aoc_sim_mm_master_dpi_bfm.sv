...
module aoc_sim_mm_master_dpi_bfm(
                                  ...
                                  agent_busy_out,
                                  done_writes_to_cra,
                                  component_control_not_ready_in,
                                  component_control_not_ready_out,
   ...
   parameter IS_MAIN_CONTROLLER         = 0;
   parameter NUM_COMPONENTS_WITH_CSR    = 0;
   ...
   parameter MBD_USE_BURST_COUNT       = 1;  // Use burstcount interface pin
   ...
   parameter COMMAND_READ_WRITE_CYCLES  = 10; // How many cycles to delay after a read/write.
   ...
   typedef enum { KERNEL_READ, KERNEL_WRITE, TERMINATE, NO_WORK, MEMORY_READ, MEMORY_WRITE, STREAM_START } Command;

   import "DPI-C" context function void __ihc_hls_dbgs(string msg);
   import "DPI-C" function Command __ihc_aoc_get_command(string component_name, int is_main_controller, output longint address,
                                                         output byte data [numContainingBytes(KI_AV_DATA_W)],
                                                         output byte byte_enable [numContainingBytes(KI_AV_NUMSYMBOLS)],
                                                         output int size, output int last_in_set, output int kernel_id);
   import "DPI-C" function void __ihc_aoc_return_ack();
   import "DPI-C" function void __ihc_aoc_return_data(byte data [numContainingBytes(KI_AV_DATA_W)], int size);
   import "DPI-C" context function int __ihc_aoc_mm_agent_read(input int id, input longint address,
                                                               output bit [MBD_AV_DATA_W-1:0] data, input int size);
   import "DPI-C" context function void __ihc_aoc_mm_agent_write(input int id, input longint address,
                                                                 input bit [MBD_AV_DATA_W-1:0] data,
                                                                 input bit [MBD_AV_NUMSYMBOLS-1:0] byte_enable,
                                                                 input int size);
   import "DPI-C" context function void __ihc_hld_dump_stream_data_rate_info();
   ...
   function int roundUpBitsToByteBoundary;
      // given an int number of bits, returns the smallest number of bits
      // that is larger than the input, and divisible by 8 
      // i.e rounds 27 to 32, 4 to 8, etc
      input int inbit;
      roundUpBitsToByteBoundary = numContainingBytes(inbit) << 3;
   endfunction

   function int numContainingBytes;
      // given an int number of bits, returns the number of bytes required to contain that number of bits
      input int inbit;
      numContainingBytes = (inbit + 7) >> 3;
   endfunction
   ...
   output [lindex(NUM_COMPONENTS_WITH_CSR):0]       agent_busy_out;
   output [lindex(NUM_COMPONENTS_WITH_CSR):0]       done_writes_to_cra;
   input  [lindex(NUM_COMPONENTS_WITH_CSR):0]       component_control_not_ready_in;
   output                                           component_control_not_ready_out;

   input                                                   ki_avm_waitrequest;
   input                                                   ki_avm_readdatavalid;
   input   [lindex(KI_AV_SYMBOL_W * KI_AV_NUMSYMBOLS):0]     ki_avm_readdata;
   output                                                    ki_avm_write;
   output                                                    ki_avm_read;
   output  [lindex(KI_AV_ADDRESS_W):0]                       ki_avm_address;
   output  [lindex(KI_AV_NUMSYMBOLS):0]                      ki_avm_byteenable;
   output  [lindex(KI_AV_BURSTCOUNT_W):0]                    ki_avm_burstcount;
   output  [lindex(KI_AV_SYMBOL_W * KI_AV_NUMSYMBOLS):0]     ki_avm_writedata;

   input                                                   mbd_avm_waitrequest;
   input                                                   mbd_avm_readdatavalid;
   input   [lindex(MBD_AV_SYMBOL_W * MBD_AV_NUMSYMBOLS):0]   mbd_avm_readdata;
   output                                                    mbd_avm_write;
   output                                                    mbd_avm_read;
   output  [lindex(MBD_AV_ADDRESS_W):0]                      mbd_avm_address;
   output  [lindex(MBD_AV_NUMSYMBOLS):0]                     mbd_avm_byteenable;
   output  [lindex(MBD_AV_BURSTCOUNT_W):0]                   mbd_avm_burstcount;
   output  [lindex(MBD_AV_SYMBOL_W * MBD_AV_NUMSYMBOLS):0]   mbd_avm_writedata;
  ...
  altera_avalon_mm_master_bfm #(
    ...
    .USE_ADDRESS                (USE_ADDRESS             ),
    .USE_BYTE_ENABLE            (USE_BYTE_ENABLE         ),
    .USE_BURSTCOUNT             (MBD_USE_BURST_COUNT     ),
    .USE_READ_DATA              (USE_READ_DATA           ),
    .USE_READ_DATA_VALID        (USE_READ_DATA_VALID     ),
    .USE_WRITE_DATA             (USE_WRITE_DATA          ),
    .USE_BEGIN_TRANSFER         (USE_BEGIN_TRANSFER      ),
  ...
  function automatic void write_to_slave(input bit [63:0] addr, input bit [lindex(KI_AV_DATA_W):0] data, input bit [lindex(KI_AV_NUMSYMBOLS):0] byteenable);
  ...
  reg [lindex(NUM_COMPONENTS_WITH_CSR):0] done_writes;
  genvar i;
  for (i = 0; i <= lindex(NUM_COMPONENTS_WITH_CSR); i = i + 1) begin
    always @(negedge reset_n) begin
      done_writes[i] <= 1'b0;
    end
    assign done_writes_to_cra[i] = done_writes[i];
  end

  assign component_control_not_ready_out = (|component_control_not_ready_in);
  ...
  task automatic read_from_memory_ip_storage(input bit [63:0] base_addr, input bit [31:0] size);
    ...
    if (size > MBD_AV_NUMSYMBOLS) begin
      ...
      while (byte_count_remaining >= 0) begin
        ...
        __ihc_aoc_mm_agent_write(0, next_addr, mem_data, {{MBD_AV_NUMSYMBOLS}{1'b1}}, MBD_AV_NUMSYMBOLS);
    ...
    if (byte_count_remaining > 0) begin
      ...
      __ihc_aoc_mm_agent_write(0, next_addr, mem_data, mem_byteenable, MBD_AV_NUMSYMBOLS);
    end

    // Return some acknowledge to the MMD layer that memcpy is done to avoid race condition
    __ihc_aoc_return_ack();
  ...
  task automatic write_to_memory_ip_storage(input bit [63:0] base_addr, input bit [31:0] size);
    ...
    if (size > MBD_AV_NUMSYMBOLS) begin
      ...
      while (byte_count_remaining >= 0) begin
        ...
        __ihc_aoc_mm_agent_read(0, next_addr, mem_data, MBD_AV_NUMSYMBOLS);
    ...
    if (byte_count_remaining > 0) begin
      ...
      __ihc_aoc_mm_agent_read(0, next_addr, mem_data, byte_count_remaining);
    ...
    // send an write acknowledge command back to MMD to say it's done writing to avoid race condition
    __ihc_aoc_return_ack();
    repeat(COMMAND_READ_WRITE_CYCLES) @(posedge clock);
  endtask
  ...
  task automatic read_commands;
    string message;
    longint address;
    typedef reg [(roundUpBitsToByteBoundary(KI_AV_NUMSYMBOLS) - 1):0] enable_unroll_array;
    typedef reg [(roundUpBitsToByteBoundary(KI_AV_DATA_W) - 1):0] data_unroll_array;
    typedef byte data_pack_array [numContainingBytes(KI_AV_DATA_W)];
    reg [(roundUpBitsToByteBoundary(KI_AV_DATA_W) - 1):0] data;
    reg [(roundUpBitsToByteBoundary(KI_AV_NUMSYMBOLS) - 1):0] byte_enable;
    byte data_array [numContainingBytes(KI_AV_DATA_W)];
    byte byte_enable_array [numContainingBytes(KI_AV_NUMSYMBOLS)];
    int size, last_in_set, kernel_id;
    // Keep track of the id of last kernel receiving STREAM_START, and if the successive
    // command is not STREAM_START for the same kernel, lower the done_writes signal
    // This allows us to keep done_writes high for consecutive cycles, achieving an ii of 1
    int last_kernel_id = -1;
    Command cmd;

    // Start reading commands
    while (1) begin
      // Don't do anything if reset is active or the MASTER BFM is telling us to wait.
      if (~reset_n || `KI_MSTR_BFM.avm_waitrequest || `MBD_MSTR_BFM.avm_waitrequest || (|component_control_not_ready_in)) begin
        if (last_kernel_id != -1) begin
          done_writes[last_kernel_id] <= 1'b0;
        end
        @(posedge clock);
        continue;
      end
      cmd = __ihc_aoc_get_command(COMPONENT_NAME, IS_MAIN_CONTROLLER, address, data_array, byte_enable_array, size, last_in_set, kernel_id);
      if (last_kernel_id != -1 && (cmd != STREAM_START || kernel_id != last_kernel_id)) begin
        done_writes[last_kernel_id] <= 1'b0;
      end
      case (cmd)
        STREAM_START:
          begin
            $sformat(message, "[%7t][msim][aoc_sim_mm_master_dpi_bfm] Done CRA args acquired for kernel id: %d", $time, kernel_id);
            __ihc_hls_dbgs(message);

            last_kernel_id = kernel_id;
            done_writes[kernel_id] <= 1'b1;
            @(posedge clock);
          end
        KERNEL_READ:
          begin
            ...
            data_array = {<<8{data_pack_array'(data)}}; 
            $sformat(message, "[%7t][msim][aoc_sim_mm_master_dpi_bfm] Kernel Read result = 0b%b", $time, data);
            __ihc_hls_dbgs(message);

            // Return it to the MMD layer.
            __ihc_aoc_return_data(data_array, (numContainingBytes(KI_AV_DATA_W)));
            repeat(COMMAND_READ_WRITE_CYCLES) @(posedge clock);
          end
        KERNEL_WRITE:
          begin
            byte_enable = enable_unroll_array'(byte_enable_array);
            data = data_unroll_array'({<<8{data_array}});
            ...
            // Takes 6 cycles for this to propagate through the kernel interface to the
            // actual CRA and be registered... :/
            // Within the kernel interface, it takes 1 cycle each in the two bridges,
            // 4 cycles in the address span expander to bridge interconnect.
            // One cycle for it to be registered seems fine, the other 5 are pretty absurd.
            @(posedge clock);
            // TODO: get rid of these extra cycles
            repeat(5) @(posedge clock);

            if (last_in_set)
            begin
              done_writes[kernel_id] <= 1;
              @(posedge clock);
              done_writes[kernel_id] <= 0;
            end

            if (COMMAND_READ_WRITE_CYCLES > 5) begin
              repeat(COMMAND_READ_WRITE_CYCLES - 6) @(posedge clock);
            end
            
            if(IS_MAIN_CONTROLLER == 0) begin
              // send an write acknowledge command back to MMD to say it's done writing to avoid race condition
              // IS_MAIN_CONTROLLER == 0 means that this isn't the main mm_master BFM  (i.e the BFM for the CRA), and we are writing to an interface
              // other than the main interface. In this case, the upstream needs to know when the write finishes
              // so that user code blocking operations can block for the appropriate amount of time.
              __ihc_aoc_return_ack();
            end
          end
        MEMORY_READ:
        ...
        MEMORY_WRITE:
          begin
            ...
            // @(negedge clock);
            $sformat(message, "[%7t][msim][aoc_sim_mm_master_dpi_bfm] Memory write address 0x%1x (size %1d)", $time, address, size);
            __ihc_hls_dbgs(message);
        ...
        TERMINATE:
          begin
            ...
            sim_done <= 1;
            @(posedge clock);
            __ihc_hld_dump_stream_data_rate_info();
            $finish;
          end
        NO_WORK:
          begin
