...
TOP_LEVEL_NAME="const_lambda_en_cfan"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [const_lambda_en_cfan::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {const_lambda_en_cfan::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [const_lambda_en_cfan::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
