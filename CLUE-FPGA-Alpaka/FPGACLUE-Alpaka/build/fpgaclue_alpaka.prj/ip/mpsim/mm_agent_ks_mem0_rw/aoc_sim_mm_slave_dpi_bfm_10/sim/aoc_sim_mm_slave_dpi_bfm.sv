...
module aoc_sim_mm_slave_dpi_bfm(
   ...
   parameter USE_WRITE_ACK            = 0;  // Use writeack interface pin
   parameter WRITE_ACK_DELAY          = 20; // Delay of the write signal
   ...
   altera_avalon_mm_slave_bfm
    ...
    // Generate write-ack signal
    generate
      if (USE_WRITE_ACK == 1) begin
        logic write_delayed[WRITE_ACK_DELAY-1:0];
        always @(posedge clock or negedge reset_n) begin
          if (!reset_n) begin
            write_delayed <= '{WRITE_ACK_DELAY{1'b0}};
          end else begin
            write_delayed[0] <= avs_write && !avs_waitrequest;
            for (int i = 1; i < WRITE_ACK_DELAY; i = i+1) begin
              write_delayed[i] <= write_delayed[i-1];
            end
          end
        end
        assign writeack = write_delayed[WRITE_ACK_DELAY-1];
      end else begin
        assign writeack = 1'b0;
      end
    endgenerate
    ...
    task automatic process_one_command;
      ...
      if (request==REQ_WRITE) begin
        ...
        for (int i = 0; i < burst_size; i++) begin
          __ihc_aoc_mm_agent_write(INTERFACE_ID, address + (i << $clog2(AV_NUMSYMBOLS)), data[i], byte_enable[i], AV_NUMSYMBOLS);
        end
        ...
        __ihc_hls_dbgs_mem(message);
        for (int i = 0; i < burst_size; i++) begin
          latency = __ihc_aoc_mm_agent_read(INTERFACE_ID, address + (i << $clog2(AV_NUMSYMBOLS)), data[i], AV_NUMSYMBOLS);
        end
        if (latency <= 1) begin
          latency = 0;
        end
